...
export default function CitasPage() {
  ...
  const guardarEdicion = async () => {
    ...
    if (traslape && traslape.length > 0) {
      setErrorEdit('⚠️ Hay un traslape con otra cita')
      ...
      {mostrarModalServicios && (
        <div className="fixed inset-0 bg-black/40 flex items-end z-50" onClick={() => setMostrarModalServicios(false)}>
          <div className="bg-white rounded-t-3xl w-full max-h-[75vh] flex flex-col p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setMostrarModalServicios(false)} className="w-full bg-teal-400 text-white py-4 rounded-2xl font-semibold shadow-md mt-4">
      ...
      {citaEditando && (
        <div className="fixed inset-0 bg-black/40 flex items-end z-50" onClick={() => setCitaEditando(null)}>
          <div className="bg-white rounded-t-3xl w-full max-h-[90vh] overflow-y-auto flex flex-col p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-800 text-lg">Editar cita</h3>
              <button onClick={() => setCitaEditando(null)} className="text-2xl text-gray-400">×</button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-4 py-4 text-left flex items-center justify-between"
                >
                  <span className={editServiciosSeleccionados.length > 0 ? 'text-gray-800 font-semibold' : 'text-gray-400'}>
              ...
              {editServiciosSeleccionados.length > 0 && (
                <div className="bg-teal-50 rounded-2xl p-4">
                  <p className="text-teal-700 font-semibold text-sm">Total: ${editValorTotal.toLocaleString()} · {editDuracionTotal} min</p>
                </div>
              )}
              ...
              <div className="flex gap-3 pb-2">
                <button onClick={() => setCitaEditando(null)} className="flex-1 bg-gray-100 text-gray-600 py-4 rounded-2xl font-semibold">
                  Cancelar
                </button>
                <button onClick={guardarEdicion} disabled={loadingEdit} className="flex-1 bg-teal-400 text-white py-4 rounded-2xl font-semibold shadow-md disabled:opacity-50">
      ...
      {mostrarModalServiciosEdit && (
        <div className="fixed inset-0 bg-black/40 flex items-end z-[60]" onClick={() => setMostrarModalServiciosEdit(false)}>
          <div className="bg-white rounded-t-3xl w-full max-h-[75vh] flex flex-col p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-800 text-lg">Elegir servicios</h3>
              <button onClick={() => setMostrarModalServiciosEdit(false)} className="text-2xl text-gray-400">×</button>
            </div>
            <div className="overflow-y-auto space-y-2 flex-1">
              {servicios.map(s => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-2xl cursor-pointer border-2 transition-all ${editServiciosSeleccionados.includes(s.id) ? 'border-teal-400 bg-teal-50' : 'border-gray-100 bg-gray-50'}`}
                >
                  <div>
                    <p className="font-semibold text-gray-800 text-sm">{s.nombre}</p>
                    <p className="text-gray-400 text-xs">${s.valor.toLocaleString()} · {s.duracion_min} min</p>
                  </div>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${editServiciosSeleccionados.includes(s.id) ? 'border-teal-400 bg-teal-400' : 'border-gray-300'}`}>
                    {editServiciosSeleccionados.includes(s.id) && <span className="text-white text-xs">✓</span>}
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setMostrarModalServiciosEdit(false)} className="w-full bg-teal-400 text-white py-4 rounded-2xl font-semibold shadow-md mt-4">
          ...
          <div className="px-4 py-4 space-y-3">
            {citasMostradas.length === 0 ? (
              ...
              citasMostradas.map((cita) => (
                <div key={cita.id} className="bg-white rounded-3xl px-5 py-4 shadow-sm relative">
                    ...
                    <div className="flex flex-col items-end gap-2 relative">
                      ...
                      {menuAbiertoId === cita.id && (
                        <>
                          ...
                          <div className="absolute right-0 top-10 bg-white rounded-2xl shadow-lg border border-gray-100 py-2 w-44 z-20">
                            <button
