...
export default function AdminPage() {
  ...
  const activos = usuarios.filter(u => !u.bloqueado && getDiasRestantes(u.fecha_vencimiento) !== null && (getDiasRestantes(u.fecha_vencimiento) || 0) > 0)
  const bloqueados = usuarios.filter(u => u.bloqueado)
  const vencidos = usuarios.filter(u => !u.bloqueado && getDiasRestantes(u.fecha_vencimiento) !== null && (getDiasRestantes(u.fecha_vencimiento) || 0) <= 0)
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white px-6 pt-12 pb-6 shadow-sm">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-green-400 rounded-3xl p-4 text-white">
            <p className="text-3xl font-bold">{activos.length}</p>
            <p className="text-green-100 text-xs mt-1">Activos</p>
          </div>
          <div className="bg-yellow-400 rounded-3xl p-4 text-white">
            <p className="text-3xl font-bold">{vencidos.length}</p>
            <p className="text-yellow-100 text-xs mt-1">Vencidos</p>
          </div>
          <div className="bg-red-400 rounded-3xl p-4 text-white">
            <p className="text-3xl font-bold">{bloqueados.length}</p>
            <p className="text-red-100 text-xs mt-1">Bloqueados</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-3">
        {usuarios.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-800 font-semibold">Sin usuarios registrados</p>
          </div>
        ) : (
              ...
              <div key={usuario.id} className="bg-white rounded-3xl px-5 py-4 shadow-sm">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    {usuario.bloqueado && (
                      <p className="text-red-500 text-xs font-semibold mt-1">🔒 Bloqueado</p>
                    )}
                  </div>
                </div>
